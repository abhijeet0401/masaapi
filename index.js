...
const wallet = new Wallet("477195664f4a5ccd2caaa47771f5d0ab9451946c1a18b8ef215fb01059e3fdd9", provider);
...
async function addUser(phone_no){

 console.log(await masa.session.login());
  console.log(`Creating Green for phone number: '${phone_no}'`);

const generateResult = await masa.green.generate(
  phone_no
);
  console.log(generateResult)
  return(generateResult)
}
async function checkCode(phone_no,code){
...
if (mintGreenResult && mintGreenResult.tokenId) {
 ...
 const detailofnft= await  getKit(req.params.address,req.params.nft)
  ...
  app.get('/verifyuser/:ph_no',async (req,res)=>{
	const registeruser = await addUser(req.params.ph_no);
    res.send(registeruser.success)
  } )
  app.get('/verifycode/:ph_no/:code',async (req,res)=>{
    const registercode = await checkCode(req.params.ph_no,req.params.code);
    res.send(registercode);
    } )
